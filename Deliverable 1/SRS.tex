...
% Begin SubSection
Climatar is a self-contained software solution with respect to the Android system it is running on. No external databases, servers, or systems are required for the system itself to work. The System itself contains layers for world containment, world component models (i.e. Climate, Greenhouse gas levels and emissions, political stances etc.), event generation, and event interaction. Depending on  the simulated world, each of the sub layers interact differently. Furthermore the System interacts with the android platform utilizing the features available to applications on android devices. The following diagram illustrates the interaction of layers within the system:


In the past Climate simulation software systems have been developed and used for some of the objectives held by Climatar. Simple solutions such as 


A climate simulation software system is not a new idea, and has infact been done in many different ways in the past. However Climatar solves unique issues not addressed by previous solutions, providing usability to a projected user base that cannot be obtained from off the shelf solutions.

Climatar is a self-contained solution for andriod 
...
\begin{enumerate}
	...
	\item Climatar is a tool to have users think about reactions to events on a global scale at an accelerated time scale.
\end{enumerate}
% End SubSection
